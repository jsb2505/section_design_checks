...
def test_ray_method_boundary_consistency():
    ...
    points = diagram.generate_diagram(n_points=120)
    ...
    for i, point in enumerate(test_points):
        ...
        _, _, is_inside, utilization = diagram.get_capacity_vector(M_Ed=point.M, N_Ed=point.N)
...
def test_scaling_property():
    ...
    _, _, _, util_base = diagram.get_capacity_vector(M_Ed=M_base, N_Ed=N_base)
    ...
    for k in scale_factors:
        ...
        _, _, _, util_scaled = diagram.get_capacity_vector(M_Ed=M_scaled, N_Ed=N_scaled)
...
def test_inside_outside_consistency():
    ...
    M_inside = 20.0
    N_inside = 50.0
    _, _, is_inside_flag, util_inside = diagram.get_capacity_vector(M_Ed=M_inside, N_Ed=N_inside)
    ...
    M_outside = 500.0
    N_outside = 1000.0
    _, _, is_outside_flag, util_outside = diagram.get_capacity_vector(M_Ed=M_outside, N_Ed=N_outside)
