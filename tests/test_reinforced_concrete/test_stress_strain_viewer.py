...
class TestStressStrainViewerHelpers:
    ...
    def test_set_section_xrange_for_equal_units(self):
        viewer = StressStrainViewer(_FakeDiagram())
        fig = _FakeFigure()
        s = _make_state(bbox=(0.0, 0.0, 300.0, 500.0))

        viewer._set_section_xrange_for_equal_units(
            fig,
            s,
            row=1,
            col=1,
        ...
        assert fig.xaxes_updates
        assert "range" in fig.xaxes_updates[-1]

    def test_set_section_xrange_guard_branches(self):
        viewer = StressStrainViewer(_FakeDiagram())
        s = _make_state()

        # plot_w <= 0 guard
        fig1 = _FakeFigure()
        fig1.layout.width = 100
        fig1.layout.margin.l = 80
        fig1.layout.margin.r = 80
        viewer._set_section_xrange_for_equal_units(fig1, s, y_range=(0.0, 500.0))
        assert fig1.xaxes_updates == []

        # missing axis guard
        fig2 = _FakeFigure()
        viewer._set_section_xrange_for_equal_units(fig2, s, col=9, y_range=(0.0, 500.0))
        assert fig2.xaxes_updates == []

        # non-positive x/y domain fractions guard
        fig3 = _FakeFigure()
        fig3.layout.xaxis.domain = [0.0, 0.0]
        viewer._set_section_xrange_for_equal_units(fig3, s, y_range=(0.0, 500.0))
        assert fig3.xaxes_updates == []

        # non-positive y-span guard
        fig4 = _FakeFigure()
        viewer._set_section_xrange_for_equal_units(fig4, s, y_range=(100.0, 100.0))
        assert fig4.xaxes_updates == []
    ...
    def test_add_subplots_and_layout_methods(self):
        ...
        assert fig6.layout.margin.b == 240
...
class TestStressStrainViewerPlotFlow:
    ...
    def test_plot_delegates_to_subplot_builders(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda self, fig, go, s, section_render="points": called.__setitem__("section", called["section"] + 1),
        )
        monkeypatch.setattr(
            StressStrainViewer,
            "_add_strain_subplot",
            lambda self, fig, go, s: called.__setitem__("strain", called["strain"] + 1),
        )
        monkeypatch.setattr(
            StressStrainViewer,
            "_add_stress_subplot",
            lambda self, fig, go, s: called.__setitem__("stress", called["stress"] + 1),
        )
        monkeypatch.setattr(
            StressStrainViewer,
